...
export const postJoin = async (req, res) => {
  ...
  if (exists) {
    return res.status(400).render("join", {
      pageTitle: "Join",
      errorMessage: "Exists email/username",
    });
  }
  try {
    await User.create({
...
export const postLogin = async (req, res) => {
  ...
  if (!user) {
    return res.status(400).render("login", {
      pageTitle,
      errorMessage: "account with username does not exists",
    });
  }
...
export const finishGithubLogin = async (req, res) => {
  ...
  if ("access_token" in tokenRequest) {
    ...
    //Github 로그인했을 때 email이 이미 등록된 이메일 경우 로그인 시켜준다.
...
export const postChangePassword = async (req, res) => {
  ...
  if (newPassword !== newPasswordConfirm) {
    return res.status(400).render("user/change-password", {
      pageTitle: "Change Password",
      errorMessage: "the new password does not match confirmation",
    });
  }
  user.password = newPassword;
  await user.save(); //hash 한다.
